...
func NormalizeCookiesToNetscape(raw string, targetURL string) string {
	...
	// 1. Check if JSON Array (e.g. from Cookie-Editor / EditThisCookie)
		...
		if err := json.Unmarshal([]byte(raw), &items); err == nil && len(items) > 0 {
			...
			for _, it := range items {
				...
				if exp, ok := it["expirationDate"].(float64); ok && exp > 0 {
					expVal = int64(exp)
				} else if expI, ok := it["expirationDate"].(int64); ok && expI > 0 {
					expVal = expI
				} else if expN, ok := it["expirationDate"].(json.Number); ok {
					if n, err := expN.Int64(); err == nil && n > 0 {
						expVal = n
					}
				}
	...
	// 2. Check if already in Netscape format
	if strings.HasPrefix(raw, "# Netscape") || strings.HasPrefix(raw, "# HTTP Cookie File") || strings.Contains(raw, "\t") {
		var sb strings.Builder
